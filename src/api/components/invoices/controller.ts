...
export = (injectedStore: typeof StoreType) => {
  ...
  const newInvoice = async (
    ...
    if (ticket) {
      app.webSocketServer?.broadcast(
        JSON.stringify({ factId: resultInsert.msg.factId, ticket: true }),
      );
      ...
      if (difTime > 10000) {
        sendAvisoFact(
          `${newFact.letra} ${zfill(newFact.pv, 5)} - ${zfill(
            newFact.cbte,
            8,
          )}`,
          newFact.nota_cred,
  ...
  const sendToPrintTicket = async (factId: number) => {
    app.webSocketServer?.broadcast(JSON.stringify({ factId, ticket: true }));
    return factId;
  };

  const sendToPrintFact = async (factId: number) => {
  ...
  const correctorImg = async () => {
    const productos: Array<INewProduct> = await store.list(
